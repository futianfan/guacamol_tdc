...
def make_mating_pool(population_mol: List[Mol], population_scores, offspring_size: int):
    ...
    if sum_scores==0:
        population_probs = [1.0/len(population_scores) for i in range(len(population_scores))]
    else:
...
class GB_GA_Generator:
    ...
    def __init__(self, smi_file, population_size, offspring_size, generations, mutation_rate):
    ...
    def generate_optimized_molecules(self):
        ...
        print("---------- before init, oracle_num", self.oracle_num, 'population_size', self.population_size)
